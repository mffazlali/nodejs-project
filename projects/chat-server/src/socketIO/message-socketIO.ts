...
export class MessageSocketIO extends SocketIO {
    ...
    send = () => {
        this.socketIO.socket.on('createMessage', async (message: MessageModel, callback) => {
            ...
            this.socketIO.io.to(user.id).emit('newMessage', new MessageModel(message.id, message.from, message.text, message.createdAt, message.latitude, message.longitude));
